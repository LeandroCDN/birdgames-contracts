...
*/
  describe("placeBet", function () {
    it("should call permit2 and deposit tokens in the treasury", async function () {
      const { gamerps, mockPermit2, woldCoin, treasury, owner, player } =
        await loadFixture(deployRPSFixture);
      ...
      await woldCoin
        .connect(player)
        .approve(mockPermit2.getAddress(), betAmount);
      ...
      const treasuryBalanceAfter = await woldCoin.balanceOf(
        treasury.getAddress()
      );
  ...
  describe("_settleBet", function () {
    it("Caller can call _settleBet", async function () {
      ...
      await woldCoin
        .connect(owner)
        .transfer(player.address, betAmount * BigInt(2));
      await woldCoin
        .connect(owner)
        .transfer(treasury.getAddress(), betAmount * BigInt(2));
      await woldCoin
        .connect(player)
        .approve(mockPermit2.getAddress(), betAmount * BigInt(200));
      const woldAddress = await woldCoin.getAddress();
      // Crear un permit simulado
      const permit = {
        permitted: {
      ...
      let totalBets: number = 300;
      for (let i = 0; i < totalBets; i++) {
        try {
          await gamerps
            .connect(player)
            .placeBet(2, permit, transferDetails, signature);
          ...
          if (argsArray.length > 5) {
            // Asegúrate de que el índice 5 existe
            ...
            console.error(
              "argsArray does not have enough elements for iteration:",
              i,
              argsArray
            );
          }
      ...
      console.log(
        "playerBalanceBefore",
        playerBalanceBefore / BigInt(10 ** 18),
        "WLD"
      );
      console.log(
        "playerBalanceAfter",
        playerBalanceAfter / BigInt(10 ** 18),
        "WLD"
      );
